...
// Pulls hook.megaPool, hook.hourlyPool(epoch), hook.lastBuyer, hook.epochLastBuyer(epoch).
...
export function useLivePoolState() {
  ...
  const res = useReadContracts({
    contracts: [
      ...
      { address: addresses.hook, abi: casteHookAbi, functionName: "epochLastBuyer", args: [epoch ?? 0n] },
    ],
    query: { enabled: epoch !== undefined, refetchInterval: 5_000 },
  });
  const [megaPool, hourlyPool, lastBuyer, epochLastBuyer] = res.data ?? [];
  return {
    epoch: epoch ?? 0n,
    ...
    epochLastBuyer: (epochLastBuyer?.result ?? null) as `0x${string}` | null,
